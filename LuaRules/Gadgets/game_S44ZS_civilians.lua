...
function gadget:GetInfo()
    return {
        ...
        desc      = "Handles civilian behavior for Corruption",
        author    = "B. Tyler (Nemo), gajop",
        date      = "December 2009, April 2015",
...
local function Flee(scaryX, scaryZ, unitID, attackerTeam) --RUN AWWAAAAAY!
	...
	if (civX > scaryX) and (civZ > scaryZ) then
		...
		--Spring.Log(LOG_SECTION, LOG_LEVEL,"outcome #1, xMax:", xMax, "zMax", zMax)
	end
	if (civX < scaryX) and (civZ < scaryZ) then
		...
		--Spring.Log(LOG_SECTION, LOG_LEVEL,"outcome #2, xMin:", xMin, "zMin", zMin)
	end
	if (civX > scaryX) and (civZ < scaryZ) then
		local xMax = civX + scaryX
		local zMin = scaryZ - civZ
		...
		z1 = math.min(zMin, civZ)
		z2 = math.max(zMin, civZ)
		--Spring.Log(LOG_SECTION, LOG_LEVEL,"outcome #3, xMax:", xMax, "zMin", zMin)
	end
	if (civX < scaryX) and (civZ > scaryZ) then
		local xMin = scaryX - civX
		local zMax = civZ + scaryZ
		...
		z1 = math.min(zMax, civZ)
		z2 = math.max(zMax, civZ)
		--Spring.Log(LOG_SECTION, LOG_LEVEL,"outcome #4, xMin:", xMin, "zMax", zMax)
	end
	--Spring.Log(LOG_SECTION, LOG_LEVEL,"x1: ",x1," x2: ",x2," z1: ",z1," z2: ",z2)
	...
	--Spring.Log(LOG_SECTION, LOG_LEVEL,"run off to", xDest, zDest, "little civilian")
...
function gadget:UnitDamaged(unitID, unitDefID, unitTeam, _, _, _, _, attackerID, _, attackerTeamID)
	if (scaredUnits[unitID] ~= nil) then
		--Spring.Log(LOG_SECTION, LOG_LEVEL,"I've been hit, run my friends!")
...
function gadget:AddScaredUnit(unitID)
    Spring.SetUnitAlwaysVisible(unitID, true)
    Spring.Log(LOG_SECTION, LOG_LEVEL,unitID, "Scared")
    scaredUnits[unitID] = 0
end

function gadget:UnitCreated(unitID, unitDefID, teamID)
	if UnitDefs[unitDefID].customParams.civilian then
        self:AddScaredUnit(unitID)
	end
end
...
function gadget:Initialize()
    ...
    for _, unitID in ipairs(Spring.GetAllUnits()) do
        if UnitDefs[Spring.GetUnitDefID(unitID)].customParams.civilian then
            self:AddScaredUnit(unitID)
        end
    end
end
...
--     end
    ...
    for teamID, duration in pairs(scaryTeams) do
        --Spring.Log(LOG_SECTION, LOG_LEVEL,"scary teamID:",teamID, "fear factor:", scaryTeams[teamID])
...
local function allytogaia()
    ...
    Spring.Log(LOG_SECTION, LOG_LEVEL,"zombieTeamID", zombieTeamID)
